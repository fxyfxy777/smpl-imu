...
class SMPLModel(Module):
  def __init__(self, device=None, model_path='basicModel_f_lbs_10_207_0_v1.0.0.pkl'):
...
def test_gpu(gpu_id=[0]):
  ...
  np.random.seed(9608)
  pose = torch.from_numpy((np.random.rand(pose_size) - 0.5) * 0.).type(torch.float64).to(device)
  betas = torch.from_numpy((np.random.rand(beta_size) - 0.5) * 0.0).type(torch.float64).to(device)
  trans = torch.from_numpy(np.zeros(3)).type(torch.float64).to(device)
  outmesh_path = './smpl_torch.obj'

  model = SMPLModel(device=device)
  result = model(betas, pose, trans)#TODO: torch.Size([10]) torch.Size([72])    torch.Size([3])
  model.write_obj(result, outmesh_path)

  from mess import display_obj
  display_obj(obj = outmesh_path)
