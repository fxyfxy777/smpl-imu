...
def render_sequence_3d(verts, faces, width, height, video_path, fps=30,
  ...
  writer.close()
def create_while_3dmesh(verts, faces, width, height, video_path, fps=30,
                       visible=False, need_norm=True, verbose=True, colors=None):
  """
  Render mesh animation using open3d.
  ...
  return mesh,vis,writer,mean,scale

def while_3dmesh_display(verts,mesh,vis,writer,mean,scale,visible=False, need_norm=True, verbose=False, colors=None):
  if need_norm:
    verts =  (verts - mean) / scale
  iterations = range(len(verts))
  if verbose:
...
def vis_mesh(verts, faces, colors=None, width=1080, height=1080):
  """
