...
class ParametricModel:
    ...
    def __init__(self, official_model_file: str, use_pose_blendshape=False, device=torch.device('cpu')):
        ...
        self.mesh_realtime = None
        self.vis_realtime=None
        self.writer_realtime= None
        self.mean_realtime = None
        self.scale_realtime = None
    ...
    def view_mesh(self, vertex_list: list, fps=60, distance_between_subjects=0.8):
        ...
        #TODO: verts.shape (bs, 6890, 3)
        # dim = 2
        # verts[:,:,dim] = verts[:,:,dim]*(-1)

        faces = np.concatenate(f_list)
        if verts.shape[0] > 1:
            # vo3d.render_sequence_3d(verts, faces, 720, 720, 'a.mp4', fps, visible=True)
            mesh,vis,writer,mean,scale = vo3d.create_while_3dmesh(verts, faces, 720, 720, 'a.mp4', fps, visible=True)
            while 1:

                verts = ( torch.rand(1, 6890, 3 ) - 0.5) * 0.4
                vo3d.while_3dmesh_display(verts,mesh,vis,writer,mean,scale)
        else:
            vo3d.vis_mesh(verts[0], faces)
    def view_motion_while_prepare(self, pose_list: list, tran_list: list = None, fps=60, distance_between_subjects=0.8):
        verts = []
        ...
        if verts.shape[0] > 1:
            # vo3d.render_sequence_3d(verts, faces, 720, 720, 'a.mp4', fps, visible=True)
            self.mesh_realtime, self.vis_realtime, self.writer_realtime, self.mean_realtime, self.scale_realtime = vo3d.create_while_3dmesh(verts, faces, 720, 720, 'a.mp4', fps, visible=True)

    def view_motion_while_display(self, pose_list: list, tran_list: list = None, fps=60, distance_between_subjects=0.8):
        verts = []
        for i in range(len(pose_list)):
            pose = pose_list[i].view(-1, len(self.parent), 3, 3)
            tran = tran_list[i].view(-1, 3) - tran_list[i].view(-1, 3)[:1] if tran_list else None
        ...
        faces = np.concatenate(f_list)
        vo3d.while_3dmesh_display(verts,self.mesh_realtime,self.vis_realtime,self.writer_realtime,self.mean_realtime,self.scale_realtime)
        # if verts.shape[0] > 1:
        #     # vo3d.render_sequence_3d(verts, faces, 720, 720, 'a.mp4', fps, visible=True)
        #     mesh,vis,writer,mean,scale = vo3d.create_while_3dmesh(verts, faces, 720, 720, 'a.mp4', fps, visible=True)
